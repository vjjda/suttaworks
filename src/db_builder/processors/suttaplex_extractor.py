...
class SuttaplexExtractor:
    """Chịu trách nhiệm đọc file suttaplex.json duy nhất và trích xuất dữ liệu."""
    ...
    def _clean_volpage_string(self, text: Any) -> str | None:
        ...
        for item in items:
            ...
            if match:
                prefix, roman, rest = match.group(1).strip(), match.group(2), match.group(3)
                try:
                    arabic_num = self._roman_to_int(roman)
                    cleaned_item = f"{prefix} {arabic_num}.{rest}" if prefix else f"{arabic_num}.{rest}"
    ...
    def execute(self):
        ...
        # Lặp qua dictionary, với key là uid và value là dữ liệu card
        for uid, card in suttaplex_dict.items():
            if not isinstance(card, dict):
                logger.warning(f"Bỏ qua mục không hợp lệ với key '{uid}' trong suttaplex.json")
                continue
            ...
            final_priority_author = priority_author[0] if isinstance(priority_author, list) and priority_author else priority_author

            self.suttaplex_data.append({
                'uid': uid, # Lấy uid từ key
