...
def process_gdrive_data(gdrive_config: dict, destination_dir: Path):
    """
    ...
    folder_url = gdrive_config.get('zip')
    ...
    version_regex = gdrive_config.get('version-date')
    latest_version = 0
    latest_file = None
    for file in files:
    ...
    # 5. Giải nén
    extract_dir_name = gdrive_config.get('extract')
