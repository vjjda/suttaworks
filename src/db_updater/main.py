...
def _setup_and_parse_args(available_modules: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        '-m', '--module', nargs='?', const='_LIST_MODULES_', default=None,
...
def _process_and_validate_args(args: argparse.Namespace, config: dict) -> tuple | None:
    ...
    if args.module == '_LIST_MODULES_':
        ...
        for mod in available_modules: print(f"- {mod}")
        return None
    ...
    modules_to_run = available_modules if args.module == 'all' else [mod.strip() for mod in args.module.split(',')]
